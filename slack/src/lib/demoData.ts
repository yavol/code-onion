...
export const approvals: Approval[] = [
  {
    id: 'perf',
    title: 'Image normalization hot path',
    layer: 'implementation.performance',
    file: 'src/scan/image_normalizer.rs',
    request: 'SIMD branch improves W-2 scan normalization by 18%. Golden images match.',
    required: ['benchmark_no_regression', 'golden_image_tests'],
  ...
  {
    id: 'pricing',
    title: 'Deluxe filing price changed',
    layer: 'money.pricing',
    file: 'src/prices/plans.json',
    request: 'Deluxe state bundle changed from $39 to $29 for early-season promo.',
    required: ['@finance', '@product', 'pricing_contract_tests'],
  ...
  {
    id: 'provider',
    title: 'OCR provider switched',
    layer: 'vendor.api_provider',
    file: 'src/providers/document_ocr.ts',
    request: 'Switch from ScanLedger to FormRead for 1099 extraction fallback path.',
  ...
  },
  {
    ...
    file: 'src/recommendations/ann_config.ts',
  ...
  },
  {
    ...
    file: 'tests/integration/checkout_paid_customer.spec.ts',
